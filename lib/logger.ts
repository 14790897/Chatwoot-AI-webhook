...
class Logger {
  ...
  info(message: string, data?: any, event: WebhookEventType | 'system' | 'ai_call' = 'system'): void {
    this.addLog({ level: 'info', message, data, event });
  }

  warn(message: string, data?: any, event: WebhookEventType | 'system' | 'ai_call' = 'system'): void {
    this.addLog({ level: 'warn', message, data, event });
  }

  error(message: string, data?: any, event: WebhookEventType | 'system' | 'ai_call' = 'system'): void {
    this.addLog({ level: 'error', message, data, event });
    this.metrics.failedRequests++;
  }

  debug(message: string, data?: any, event: WebhookEventType | 'system' | 'ai_call' = 'system'): void {
  ...
  // 获取日志
  getLogs(limit?: number, level?: LogEntry['level'], event?: string): LogEntry[] {
  ...
  getHealthStatus(): {
    ...
    const checks = {
      hasRecentActivity: Date.now() - new Date(this.metrics.lastActivity).getTime() < 300000, // 5分钟内有活动
      lowErrorRate: this.metrics.totalRequests === 0 || (this.metrics.failedRequests / this.metrics.totalRequests) < 0.1, // 错误率低于10%
      aiCallsWorking: this.metrics.aiCallsCount === 0 || (this.metrics.aiCallsSuccess / this.metrics.aiCallsCount) > 0.8, // AI调用成功率高于80%
    };
